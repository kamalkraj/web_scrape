...
class GoogleSearchAPI:
    def __init__(self):
        ...
        if not self.cx:
            raise ValueError(
                "CX (Search Engine ID) not found. Set the cx environment variable."
            )
        self.client = httpx.AsyncClient(base_url=self.base_url, headers=self.headers)

    async def response(
        self, method: str, query: str, max_results: int = 10
    ) -> SearchResults:
        if not query:
        ...
        while total_fetched < max_results:
            num_results = min(
                max_results - total_fetched, 10
            )  # API allows max 10 results at a time
            ...
            try:
                ...
                if "nextPage" in response.json()["queries"]:
                    start_index = response.json()["queries"]["nextPage"][0][
                        "startIndex"
                    ]
        ...
        formatted_results = [
            self.results(result, method) for result in all_results[:max_results]
        ]
    ...
    def results(self, result: Dict, search_type: str) -> Dict:
        if search_type == "image":
            ...
            return {
                ...
                "title": self.field_search(
                    result,
                    "pagemap.metatags.0.og:title",
                    "pagemap.metatags.0.twitter:title",
                    "title",
                    "pagemap.metatags.0.og:image:alt",
                    "pagemap.metatags.0.twitter:image:alt",
                ),
                "snippet": self.field_search(
                    result,
